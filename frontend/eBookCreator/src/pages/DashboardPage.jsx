...
const BookCardSkeleton = () => (
  <div className="animate-pulse bg-white border border-slate-200 rounded-lg shadow-sm ">
    <div className="w-full aspect-[18/27] bg-slate-200 rounded-t-lg"></div>
    <div className="p-4">
...
const DashboardPage = () => {
  ...
  const [bookToDel, setBookToDel] = useState(null);
  const { user } = useAuth();
    ...
    if (!bookToDel) return;
        try {
            await axiosInstance.delete(
                `${API_PATHS.BOOKS.DELETE_BOOK}/${bookToDel}`
            );
            setBooks(books.filter((book) => book._id !== bookToDel));
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {books.map((book) => (
              <BookCard
                key={book._id}
                book={book}
                onDelete={() => setBookToDel(book._id)}
              />
            ))}
          </div>
        )}
        ...
        <ConfirmationModal
            isOpen={!!bookToDel}
            onClose={() => setBookToDel(null)}
            onConfirm={handleDeleteBook}
            title="Delete Book"
            message="Are your sure you want to delete this this eBook? This action cannot be undone."
        />
