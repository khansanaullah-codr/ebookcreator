    ...
    try {
      if (token.type === "heading_open") {
        ...
        if (nextToken && nextToken.type === "inline") {
          ...
          i++; // Skip the inline and heading_close
        }
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        try {
            if (token.type === "heading_open") {
                ...
                if (i + 1 < tokens.length && tokens[i + 1].type === "heaidng_close") {
                    i++;
                }
            } else if (token.type === "paragraph_open") {
                doc
                ...
                listType = null;
                doc.moveDown(
                    TYPOGRAPHY.paragraphSpacing / TYPOGRAPHY.sizes.body
                );
            } else if (token.type === "ordered_list_open") {
                inList = true;
                listType = "ordered";
